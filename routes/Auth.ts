...
router.post('/create-user', async (req: Request, res: Response) => {
  ...
  // If the requested username, email, and password meet all the requirements (no special characters in username, password must be at least 8 characters, and the password contains special characters) then continue on, else send an error message
  if (valid_name(name)) {
    if (
      validateEmail(email) &&
      password.length >= 8 &&
      ...
      const fullURL = `${CurrentServer}/submit-verification` // The verification token url
...
router.get('/submit-verification', async (req: Request, res: Response) => {
  ...
  if (veri_data.hasExpired == 'false') {
    ...
    const defaultPfp = `${CurrentServer}/pfps/default.jpg`
